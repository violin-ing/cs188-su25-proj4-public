...
def joinFactors(factors: List[Factor]):
    ...
    "*** YOUR CODE HERE ***"
    # Note to self: for P(A|B,C), A is unconditioned, B and C are conditioned

    factorsList = list(factors)

    # Get all unconditioned and conditioned vars
    unconditionedVars = set()
    conditionedVars = set()
    for factor in factorsList:
        unconditionedVars |= set(factor.unconditionedVariables()) # Update the original set directly
        conditionedVars |= set(factor.conditionedVariables())

    # Make sure no conditioned var is also unconditioned
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        newFactor = Factor(newUnconditioned, newConditioned, varDomains)

        # Iterate over all assignments of the new factor
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            totalProb = 0.0
            for val in varDomains[eliminationVariable]:
