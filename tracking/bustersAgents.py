    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # Find most likely position for each living ghost
        ghostPos = []
        for dist in livingGhostPositionDistributions:
            mostLikely = dist.argMax()
            ghostPos.append(mostLikely)
        ...
        # Choose the best action to approach the closest ghost
        bestAction = None
        minSuccDist = float('inf')
        for action in legal:
            succPos = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(succPos, closestGhostPos)
            if dist < minSuccDist:
                minSuccDist = dist
                bestAction = action
