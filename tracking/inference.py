...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(PAC, OBS0), (PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS0)]

    allPossPos = [(i, j) for i in range(X_RANGE) for j in range(Y_RANGE)]
    variableDomainsDict[PAC] = allPossPos
    variableDomainsDict[GHOST0] = allPossPos
    variableDomainsDict[GHOST1] = allPossPos

    maxObs = manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)) + MAX_NOISE
    obsDomain = list(range(0, maxObs + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminatedVal in eliminationOrder:
            unjoinedFactors, joinedFactors = joinFactorsByVariable(currFactors, eliminatedVal)
            currFactors = unjoinedFactors

            if len(joinedFactors.unconditionedVariables()) > 1:
                currFactors.append(eliminate(joinedFactors, eliminatedVal))

        return normalize(joinFactors(currFactors))
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Ghost is in jail -> only valid observation is None
        if ghostPosition == jailPosition:
            return 1.0 if noisyDistance is None else 0.0 # P = 0 for invalid observation
        
        # Ghost is out, but we observed nothing -> invalid
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Observation update: P(G_t | O_t) ∝ P(O_t | G_t) * P(G_t)
        # where P(G_t) = current belief, P(O_t | G_t) = the observation model
        ...
        for ghostPos in self.allPositions:
            res = self.getObservationProb(observation, pacmanPos, ghostPos, jailPos) * currBeliefs[ghostPos]
            newBeliefs[ghostPos] = res
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Time-elapse update: P(G_t) = Sum(P(G_t | G_t-1) * P(G_t-1))
        currBeliefs = self.getBeliefDistribution()
        newBeliefs = DiscreteDistribution()
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += currBeliefs[oldPos] * prob
